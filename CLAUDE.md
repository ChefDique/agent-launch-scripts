...
**Atlas migration heads-up.** AgentRemote's product evolution moves to the Atlas-of-Atlas-Island agent post-session (ARB-003/004/008/009 in CorporateHQ). This Claude (TMUX-MASTA / operator-station meta-agent) retains the tmux script management lane (`chq-tmux.sh`, `launch-agent.sh`, `agents.json`) and any infra-touching AgentRemote work. Atlas owns the renderer / aesthetic / feature direction going forward. ALS-001 closes the post-port rewiring (voice, keybindings, gradient cleanup, drawer overlap) under this Claude's lane; future "polish pass" / new feature work should route to Atlas.

Closed in this session (ALS-001):
- Voice/Whisper STT — hold-key-1-5 records, releases transcribes via local whisper, broadcasts to that single agent. Esc / window-blur cancel without sending. Permission prompt handled at first hold via `getUserMedia`.
- Keybindings audited + documented in a comment block at the top of the renderer script. 1-5 toggle/voice, Esc priority stack (voice > add-form > tile-popover > drawer), Cmd+Shift+Space summon, Enter sends.
- Iridescent-rim no longer bleeds through the chat input area. Chat-wrap uses solid `rgba(14,14,22,0.92)` fill; bordered iridescent glass-fx is now near-opaque (0.985 alpha).
- BrowserWindow grown to a 860×900 transparent canvas with `setIgnoreMouseEvents(true,{forward:true})`. Renderer flips ignore=false when cursor enters painted regions, so empty zones click through to underlying apps. Drawer + add-form now extend past the panel without clipping. Drag region scoped to `.panel-shell` only — body is pointer-events:none.

Open backlog (route to Atlas going forward):
- Polish pass — multiplane Z-stacking glitches, menu animation styling, SVG avatar fidelity, popover (X) close affordances, animation timing tightening.
