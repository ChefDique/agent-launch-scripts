...
| Code fixed; needs Richard live verification | Pet chat should show only the intended chat/agent stream, not the input box, prompt chrome, statusline, full terminal screen, or Codex TUI thinking/tool transcript lines. | Pet chat view contains clean conversation/output stream only. Team chat mention filtering ignores unrelated agents merely mentioning the agent by name, and renderer tests cover Codex TUI noise filters. |
