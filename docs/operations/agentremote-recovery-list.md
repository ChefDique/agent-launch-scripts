...
| Code fixed; needs Richard live verification | Pet chat should show only current intended chat/agent stream, not old team-chat history, input box, prompt chrome, statusline, full terminal screen, or Codex/Claude TUI thinking/tool transcript lines. | Pet chat now starts at team-chat EOF and only renders new matching envelopes; raw tmux pane streaming is registry opt-in via `pet_pane_stream: true` or `pet_chat_source: "pane"`. Team chat mention filtering ignores unrelated agents merely mentioning the agent by name, and renderer tests cover the stream/history gates plus TUI noise filters. |
