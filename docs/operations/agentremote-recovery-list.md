...
| Code fixed; needs Richard live verification | Codex model dropdowns should follow the local Codex config/env source instead of stale hardcoded GPT entries. | Open add-agent or settings model picker for a Codex agent and confirm it lists the current local Codex model, currently `gpt-5.5` from `~/.codex/config.toml` unless an explicit `AGENTREMOTE_CODEX_MODELS` / `SWARMY_CODEX_MODELS` allowlist is set. Source is `remote-app/config/harness-models.json` via `remote-app/harness-models.js` and `get-harness-models` IPC; `launch-agent.sh` rejects unsupported Codex model ids before exec. |
