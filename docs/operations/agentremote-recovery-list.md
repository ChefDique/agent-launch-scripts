...
| Code fixed in v1.4.7; needs Richard live verification | Image/text paste into the embedded xterm viewer should not forward `Ctrl+V` garbage like `0x16`; it should paste clipboard text or an image reference into the target pane. | Open an agent terminal panel, paste text and an image, confirm pane receives readable content. Static renderer test now covers Cmd/Ctrl+V, Ctrl+Shift+V, and `0x16` fallback through the image/text paste helper. |
| Code fixed in v1.4.9; needs Richard live verification | Embedded xterm input should support normal macOS terminal editing ergonomics: Option/Alt+Left and Option/Alt+Right move by word, Option/Alt+Backspace deletes the previous word, forward Delete deletes the next word, and mouse selection works without dragging the HUD. | Open an agent terminal panel and verify word navigation/delete plus mouse text selection. Renderer static tests cover the shortcut bytes, paste-path preservation, and xterm drag-safe selection options. |
...
| Code fixed; needs Richard live verification | Codex model dropdowns should follow the local Codex config/catalog/env source instead of collapsing to one hardcoded GPT default. | Open add-agent or settings model picker for a Codex agent and confirm it lists `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.3-codex-spark`, and `gpt-5.2` unless an explicit `AGENTREMOTE_CODEX_MODELS` / `SWARMY_CODEX_MODELS` allowlist is set. Source is `~/.codex/models_cache.json` plus `remote-app/config/harness-models.json` via `remote-app/harness-models.js` and `get-harness-models` IPC; `launch-agent.sh` rejects unsupported Codex model ids before exec. |
