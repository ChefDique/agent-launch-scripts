...
- **Non-regression spec** — mined `memory/sessions/` (8 files) + LEARNINGS with subagents, diffed against the live code, and consolidated 23 requirements across 7 collision clusters. Caught a live landmine: the `tmux-masta.md` OSC-1337 image-paste note is a stale failed-session diagnosis (code uses a `[image: /path]` text reference) — recorded so it can't trigger a regression.
