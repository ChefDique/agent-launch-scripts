...
test('floating pet chat uses clean team chat stream and supports pasted images', () => {
  ...
  assert.match(petWindow, /function normalizePaneLine/);
  ...
  assert.match(petWindow, /\^\(Called\|Calling\|Running\|Processing\|Bashed\|Globbed\|Grep/);
  assert.match(petWindow, /text\.length < 4/);
  assert.match(petWindow, /Agent\\s\+\\\"\.\+\\\"\\s\+\(completed\|launched\)/);
