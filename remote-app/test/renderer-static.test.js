...
test('agent pets are per-agent and use the Codex pet roster instead of a hard-coded companion', () => {
  ...
  assert.doesNotMatch(html, /agent\.id === 'neo'/);
...
test('dock labels and chat identity are registry-driven, not hard-coded by agent id', () => {
  ...
  assert.doesNotMatch(main, /agent\.id === 'neo'/);
});
...
test('floating pet chat uses clean team chat stream and supports pasted images', () => {
  ...
  assert.doesNotMatch(main, /payload && payload\.history === false/);
  assert.match(petWindow, /ipcRenderer\.invoke\('chat-tail-init'\)/);
  ...
  assert.match(petWindow, /function paneRenderLimit\(\)/);
  assert.match(petWindow, /petStreamProfile \? config\.agent\.petStreamProfile : DEFAULT_PROFILE\.id/);
  assert.match(petWindow, /petStreamProfileOptions/);
  assert.match(petWindow, /paneFilterState = createPaneStreamFilterState\(\)/);
  assert.match(petWindow, /cleanPaneLines\(res\.lines, \{ profile: paneStreamProfile\(\) \}\)/);
  assert.doesNotMatch(petWindow, /Opus\|Sonnet\|Claude\|Codex\|GPT/);
  assert.doesNotMatch(petWindow, /\^\(Ran\|Read\|Explored\|Edited/);
