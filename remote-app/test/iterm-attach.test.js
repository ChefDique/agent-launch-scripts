...
test('iTerm attach script targets the newly created iTerm tab before writing attach', () => {
  const script = buildITermAttachScript('bash /Users/richardadair/agent-launch-scripts/chq-tmux.sh attach');
  ...
  assert.match(script, /write text "bash \/Users\/richardadair\/agent-launch-scripts\/chq-tmux\.sh attach"/);
...
test('iTerm first-window attach script writes into the existing first window', () => {
  const script = buildITermFirstWindowAttachScript('bash /Users/richardadair/agent-launch-scripts/chq-tmux.sh attach');
